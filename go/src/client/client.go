...
func (g makeRandomBytesIO) send() inputT {
	n, err := rand.Read(g.sliceToFill)
	return g.msg(g.sliceToFill, n, err)
}

type readFileIO struct {
	filePath string
	msg func([]byte, error) inputT
}

func (r readFileIO) send() inputT {
	content, err := ioutil.ReadFile(r.filePath)
	return r.msg(content, err)
}

type nextFormPartIO struct {
	multipartReader *multipart.Reader
	msg func(*multipart.Part, error) inputT
}

func (n nextFormPartIO) send() inputT {
	filepart, err := n.multipartReader.NextPart()
	return n.msg(filepart, err)
}

type getFormNameIO struct {
	multipartReader *multipart.Part
	msg func(string) inputT
}

func (g getFormNameIO) send() inputT {
	return g.msg(g.multipartReader.FormName())
}

type readAllIO struct {
	r io.Reader
	msg func([]byte, error) inputT
}

func (r readAllIO) send() inputT {
	data, err := ioutil.ReadAll(r.r)
	return r.msg(data, err)
}

type getFormReaderIO struct {
	r *http.Request
	msg func(*multipart.Reader, error) inputT
}

func (g getFormReaderIO) send() inputT {
	multipartReader, err := g.r.MultipartReader()
	return g.msg(multipartReader, err)
}

type makeFileHandleIO struct {
	filepath string
	msg func(*os.File, error) inputT
}

func (m makeFileHandleIO) send() inputT {
	fileHandle, err := os.Create(m.filepath)
	return m.msg(fileHandle, err)
}

type makeTeeIO struct {
	r io.Reader
	w io.Writer
	msg func(io.Reader) inputT
}

func (m makeTeeIO) send() inputT {
	return m.msg(io.TeeReader(m.r, m.w))
}

type ioCopyIO struct {
	w io.Writer
	r io.Reader
	msg func(int64, error) inputT
}

func (i ioCopyIO) send() inputT {
	n, err := io.Copy(i.w, i.r)
	return i.msg(n, err)
}

type readHasherIO struct {
	hasher hash.Hash
	msg func([]byte) inputT
}

func (r readHasherIO) struct {
	return r.msg(hasher.Sum(nil))
}

type renameFileIO struct {
	fromPath string
	toPath string
	msg func(error) inputT
}

func (r renameFileIO) send() inputT {
	return r.msg(os.Rename(r.fromPath, r.toPath))
}

type writeFileIO struct {
	filePath string
	data []byte
	perm os.FileMode
	msg func(error) inputT
...
func (w writeFileIO) send() inputT {
	return w.msg(ioutil.WriteFile(w.filePath, w.data, w.perm))
}

type writeToWriterIO struct {
	w io.Writer
	data []byte
	msg func(int, error) inputT
}

func (w writeToWriterIO) send() inputT {
	n, err := w.w.Write(w.data)
	return w.msg(n, err)
}

type emptyStructInChanIO struct {
	ch chan struct{}
	msg func() inputT
}

func (e emptyStructInChanIO) send() inputT {
	e.ch <- struct{}{}
	return e.msg()
}

type readMainChansIO struct {
	httpChan chan httpInputT
	httpMsg func(httpInputT) inputT
	tcpInChan chan common.ClientToClient
	tcpInMsg func(common.ClientToClient) inputT
}

func (h httpInputT) update(s *stateT) (stateT, outputT) {
	return *s, readHttpParamIO{
		h.r,
		"securityCode",
		func(securityCode string) inputT {
			return httpSecurityCodeT{h, securityCode}
		},
	}
}

type httpSecurityCodeT struct {
	httpInput httpInputT
	securityCode string
}

func (h httpSecurityCodeT) update(s *stateT) (stateT, outputT) {
	return *s, readHttpParamIO{
		h.httpInput.r,
		"subRoute",
		func(subRoute string) inputT {
			return httpSubRouteT{h.httpInput, subRoute}
		},
	}
}

type httpSubRouteT struct {
	httpInput httpInputT
	subRoute string
}

func (h httpSubRouteT) update(s *stateT) (stateT, outputT) {
	subRoute := ""
	if _, ok := subRouteApps[h.httpInput.route]; ok {
		subRoute = h.subRoute
	}
	if h.httpInput.route == "saveapp" {
		return *s, getFormReaderIO{
			h.httpInput.r,
			func(reader *multipart.Reader, err error) inputT {
				return httpFormReaderT{
					h.httpInput,
					h.subRoute,
					*reader,
					err,
...
type httpFormReaderT struct {
	httpInput httpInputT
	subRoute string
	bodyFileReader multipart.Reader
	err error
}

func (h httpFormReaderT) update(s *stateT) (stateT, outputT) {
	if h.err != nil {
		return *s, defaultIO(s)
	}
	return *s, nextFormPartIO{
		&h.bodyFileReader,
		func(nextPart *multipart.Part, err error) inputT {
			return httpNextPartT{
				h.httpInput,
				h.subRoute,
				*nextPart,
				err,
...
func (h httpNextPartT) update(s *stateT) (stateT, outputT) {
	if h.err != nil {
		return *s, defaultIO(s)
	}
	return *s, getFormNameIO{
		&h.nextPart,
		func(formName string) inputT {
			return httpFormNameT{
				h.httpInput,
				h.subRoute,
				h.nextPart,
				formName,
...
const bytesInTmpFileName = 16

func (h httpFormNameT) update(s *stateT) (stateT, outputT) {
	if h.formName != "file" {
		return *s, defaultIO(s)
	}
	return makeRandomBytesIO{
		make([]byte, bytesInTmpFileName),
		func(filledSlice []byte, bytesRead int, err error) inputT {
			return httpTmpFileNameT{
				h.httpInput,
				h.subRoute,
				h.nextPart,
				filledSlice,
...
func (h httpTmpFileNameT) update(s *stateT) (stateT, outputT) {
	if h.err != nil {
		return *s, defaultIO(s)
	}
	if h.bytesRead != bytesInTmpFileName {
		return *s, defaultIO(s)
	}
	tmpFilename := base64.URLEncoding.EncodeToString(h.randomBytes)
	tmpPath := s.dataDir + "/tmp/" + tmpFilename
	return *s, makeFileHandleIO{
		tmpPath,
		func(fileHandle *os.File, err error) inputT {
			return httpTmpFileHandleT{
				h.httpInput,
				h.subRoute,
				h.nextPart,
				fileHandle,
...
type httpTmpFileHandleT struct {
	httpInput httpInputT
	subRoute string
	nextPart multipart.Part
	fileHandle *os.File
...
func (h httpTmpFileHandleT) update(s *stateT) (stateT, outputT) {
	if h.err != nil {
		return *s, defaultIO(s)
	}
...
func (h httpTeeOnT) update(s *stateT) (stateT, outputT) {
	return ioCopyIO{
		h.fileHandle,
		h.tee,
		func(n int64, err error) inputT {
			return httpWrittenToTmpFileT{
				h.httpInput,
				h.subRoute,
				n,
				err,
...
type httpWrittenToTmpFileT struct {
	httpInput httpInputT
	subRoute string
	bytesCopied int64
	err error
...
func (i httpWrittenToTmpFileT) update(s *stateT) (stateT, outputT) {
	if i.err != nil {
		return *s, defaultIO(s)
	}
	return readHasherIO{
		i.hasher,
		func(hash []byte) inputT {
			return httpGotAppHashT{
				i.httpInput,
				i.subRoute,
				hash,
				i.tmpPath,
...
func (r readHasherIO) update(s *stateT) (stateT, outputT) {
	hash := base64.URLEncoding.EncodeToString(r.hash)
	return
}

	// return *s, readAllIO{
	// 	h.r.Body,
	// 	func(body []byte, err error) inputT {
	// 		return httpBodyT{httpInput, body, err}
	// 	},
	// }


// type httpBodyT struct {
// 	httpInput httpInputT
// 	body []byte
// 	err error
// }
// 
// func (h httpBodyT) update(s *stateT) (stateT, outputT) {
// 	 
// }

func (r readMainChansIO) send() inputT {
	select {
	case h := <-r.httpChan:
		return r.httpMsg(h)
	case t := <-r.tcpInChan:
...
type readHttpParamIO struct {
	r *http.Request
	name string
	msg func(string) inputT
}

func (r readHttpParamIO) send() inputT {
	return r.msg(pat.Param(r.r, r.name))
}
...
func (p printIO) send() inputT {
	fmt.Println(p.toPrint)
	return p.msg()
}

type sendHttpErrIO struct {
...
func (s sendHttpErrIO) send() inputT {
	http.Error(s.w, s.errMsg, s.httpCode)
	return s.msg()
}

type getPosixTimeIO struct {
	msg func(int64) inputT
}

func (g getPosixTimeIO) send() inputT {
	return g.msg(time.Now().Unix())
}

type tcpDialIO struct {
	address string
	msg func(net.Conn, error) inputT
}

func (t tcpDialIO) send() inputT {
	conn, err := net.Dial("tcp", t.address)
	return t.msg(conn, err)
}

type newGobEncoderIO struct {
	conn net.Conn
	msg func(*gob.Encoder) inputT
}

func (n newGobEncoderIO) send() inputT {
	return n.msg(gob.NewEncoder(n.conn))
}

type newGobDecoderIO struct {
	conn net.Conn
	msg func(*gob.Decoder) inputT
}

func (n newGobDecoderIO) send() inputT {
	return n.msg(gob.NewDecoder(n.conn))
}
...
func defaultIO(s *stateT) readMainChansIO {
	return readMainChansIO{
		s.httpChan,
		func(httpInput httpInputT) inputT { return httpInput },
		s.tcpInChan,
		func(cToC common.ClientToClient) inputT { return cToC },
	}
}
