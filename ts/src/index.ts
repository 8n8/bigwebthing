(function () {
    ...
    async function getPowInfo(): Promise<[PowInfo, string]> {
        ...
        if (response.length !== 9) {
            return [
                nullPowInfo,
                "powInfo response is not 9 bytes long, it is " +
                    response.length,
            ];
        }
    ...
    async function getEncryptionKey(
        ...
        if (response.length !== 96) {
            return [
                nullUint8Array(),
                "signed encryption key is not 96 bytes long",
            ];
        }
        ...
        if (encryptionKey === null) {
            return [
                nullUint8Array(),
                "could not verify encryption key signature",
            ];
        }
    ...
    async function getRecipientSigningKey(
        ...
        if (key === undefined) {
            return [
                nullUint8Array(),
                "no signing key for recipient " + recipient,
            ];
        }
        return [key, ""];
    }
    ...
    async function lookupSigningKey(
        author: number
    ...
    async function validateReceipt(
        ...
        const signed: Uint8Array | null = signOpen(
            rawChunk.chunk.slice(5),
            key
        );
        if (signed === null) {
    ...
    async function joinChunks(
        ...
        if (!equalBytes(hash, chunks[0].totalHash)) {
            return [
                nullUint8Array(),
                "assembled chunk did not match expected hash",
            ];
        }

        const author: number = chunks[0].author;
        const receiptErr: string = await sendReceipt(
            hash,
            author,
            keys,
            myName
    ...
    async function unpackOneDownload(
        ...
        const [relevantChunks, newUsed]: [
            Chunk[],
            Set<number>
        ] = getRelevantChunks(decodedDownloads, start.totalHash);
        ...
        const [completeMessage, joinErr]: [
            Uint8Array,
            string
        ] = await joinChunks(sortedChunks, keys, myName);
    ...
    async function unpackDownloads(
        ...
        const numDownloads: number = rawDownloads.length;
        const [decodedDownloads, decodeErr]: [
            Chunk[],
            string
        ] = await decodeChunks(rawDownloads);
        if (decodeErr !== "") {
            return [[], [], decodeErr];
        }
    ...
    async function handleWhitelistRequest(id: number): Promise<string> {
        ...
        const whitelistErr: string = await sendWhitelistRequest(
            id,
            keys,
            myName
        );
        ...
        const signingKey: [Uint8Array, string] = await getRecipientSigningKey(
            id
        );
        if (signingKey[1] !== "") {
            return signingKey[1];
    ...
    async function sendMessageHelp(rawB64: string): Promise<void> {
        ...
        const sendError: string = await sendClientToClient(
            asBytes,
            keys,
            myName
        );
    ...
    // This function is mainly copied from the code that is
    // automatically generated by wasm-pack, for interacting
    // with WASM generated from Rust.
    async function runWasm(toRun: ToRun): Promise<Uint8Array> {
        const moduleBuffer = toBytes(toRun.wasm);
        const module_ = await WebAssembly.compile(moduleBuffer);
        const wasm = await WebAssembly.instantiate(module_, {});
        ...
        return big_web_thing(toRun.userInput);
    }
